var b2Vec2 			= b2Vec2 || Box2D.Common.Math.b2Vec2,
	b2FixtureDef 	= b2FixtureDef || Box2D.Dynamics.b2FixtureDef,
	b2PolygonShape 	= b2PolygonShape || Box2D.Collision.Shapes.b2PolygonShape,
	b2CircleShape 	= b2CircleShape || Box2D.Collision.Shapes.b2CircleShape,
	b2Body 			= b2Body || Box2D.Dynamics.b2Body,
	b2BodyDef 		= b2BodyDef || Box2D.Dynamics.b2BodyDef;
...
rocket88.PhysicsComponent = rocket88.Component.extend({
	...
	init: function() {
		...
		this.__defineSetter__("isKinematic", function(isKinematic) { 
			if(this.isReady) {
				this._body.setType(isKinematic ? "kinematic" : "dynamic");
			}
		});
		...
		this.__defineSetter__("isFixedRotation", function(isFixedRotation) { 
			if(this.isReady) {
				this._body.SetFixedRotation(isFixedRotation);
			}
		});
	...
	ready: function() {
		...
		// Create body
		this._body = rocket88.director.scene.world.CreateBody(myBody);
	},
